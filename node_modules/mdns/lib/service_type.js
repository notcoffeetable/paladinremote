...
ServiceType.prototype.toString = function() {
  ...
  if (this.subtypes.length > 0) {
    // XXX subtypes do not have to start with an underscore, but escaping must happen here
...
ServiceType.prototype.fromString = function fromString(string) {
  var is_wildcard = string === ServiceType.wildcard
    , subtypes = string.split(',') // XXX this will break with escaped commas...
  ...
  // XXX subtypes have different format rules ... fix this
  ...
  // XXX subtypes do not have to start with an underscore, but unescaping is needed here
...
var escapeSubtype = exports.escapeSubtype = function escapeSubtype(str) {
}

var extractSubtypes = exports.extractSubtypes = function extractSubtypes(str) {
  var i = str.indexOf(',')
    , subtypes = []
    ;
  if (i === -1) {
  ...
  var subtype_string = str.substr(i+1)
    , subtype = ''
    , preceededByBackslash = false
    ;
  for (i = 0; i < subtype_string.length; ++i) {
    if (preceededByBackslash) {
      switch (subtype_string[i]) {
        case '\\':
        case ',':
          subtype += subtype_string[i]
          break;
      }
      preceededByBackslash = false;
    } else {
      switch (subtype_string[i]) {
        case ',':
          if (subtype.length > 63) {
            throw new Error("subtype '" + subtype + "' is longer than 63 bytes");
          }
          subtypes.push(subtype);
          subtype = '';
          break;
        case '\\':
          preceededByBackslash = true;
          break;
        case '.':
          throw new Error('dots must be escaped by a backslash in subtypes');
          break;
        // TODO zero bytes
        default:
          subtype += subtype_string[i];
